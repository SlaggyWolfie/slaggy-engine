...
namespace slaggy
{
	...
	bool StaticOctree::insert(Shape* shape)
	{
		if (_nodes[0] == nullptr)
		{
		...
		glm::uvec3 uxn = glm::uvec3(glm::sign(distMax) / 2.0f + 0.5f);
		...
		{
			const unsigned power = static_cast<unsigned>(std::pow(2, i));
			octantMax += uxn[i] * power;
			octantMin += unn[i] * power;
		}

		// (true) we found it, boys!
		if (octantMax == octantMin) /*insert*/ return _nodes[octantMax]->insert(shape);
